   ...
   <main className="flex min-h-screen items-center justify-center bg-surface-hover p-4">
    <div className="w-full max-w-sm">
     ...
     <section className="rounded border border-border-strong bg-surface p-8 shadow-md text-center">
      ...
      <h2 className="mb-2 text-base font-semibold text-fg">{t('reset_request_sent_title')}</h2>
      <p className="mb-4 text-sm text-fg-muted">{t('reset_request_sent_desc')}</p>
      <p className="text-xs text-fg-faint mb-6">{t('reset_request_sent_note')}</p>
      <button
       type="button"
   ...
   <div className="w-full max-w-sm">
    ...
    <section className="rounded border border-border-strong bg-surface shadow-md">
     <div className="border-b border-border bg-surface-2 px-5 py-3">
      <h2 className="text-sm font-semibold text-fg-muted">{t('reset_request_title')}</h2>
     </div>
     <div className="p-5">
      <p className="mb-4 text-sm text-fg-muted">{t('reset_request_desc')}</p>
      <form className="space-y-4" onSubmit={(event) => void onSubmit(event)}>
       <div>
        <label className="mb-1 block text-xs font-semibold text-fg-muted">{t('reset_request_username_label')}</label>
        <input
         type="text"
         value={username}
       ...
       <div>
        <label className="mb-1 block text-xs font-semibold text-fg-muted">{t('reset_request_note_label')}</label>
        <textarea
         value={note}
