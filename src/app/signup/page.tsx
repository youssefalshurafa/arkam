...
export default function SignupPage() {
 ...
 if (status === 'authenticated') {
  return (
   <main
    className="min-h-screen px-4 py-5 text-slate-900 sm:px-6 lg:px-8"
    style={{
     background:
   ...
   <section className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-900/95 p-6 text-slate-100 shadow-[0_24px_60px_-32px_rgba(15,23,42,0.35)] sm:p-8 lg:p-9">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
     <div>
      ...
      <h2 className="mt-4 text-2xl font-semibold tracking-tight text-white">Create account</h2>
      <p className="mt-2 max-w-md text-sm leading-6 text-slate-300">Start with email/password or continue with Google.</p>
     </div>
     <button
      type="button"
      onClick={() => router.push('/login')}
      className="inline-flex w-full shrink-0 items-center justify-center rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/10 sm:w-auto"
     >
      Sign in
     </button>
    </div>
    ...
    >
     <div className="space-y-2">
      <label className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-300">Full name</label>
      <input
       type="text"
       value={fullName}
     ...
     <div className="space-y-2">
      <label className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-300">Password</label>
      <div
       className="relative"
     ...
     <div className="space-y-2">
      <label className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-300">Workspace name</label>
      <input
       type="text"
       value={workspaceName}
