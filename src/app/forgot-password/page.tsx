 ...
 if (emailSent) {
  return cardShell(
   <section className="rounded border border-border-strong bg-surface p-8 shadow-md text-center">
    <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-accent-weak text-4xl">
     ✉️
    </div>
    <h2 className="mb-2 text-base font-semibold text-fg">{t('forgot_password_sent_title')}</h2>
    <p className="mb-1 text-sm text-fg-muted">{t('forgot_password_sent_line1')}</p>
    <p className="mb-4 text-sm font-semibold text-fg break-all">{identifier}</p>
    <p className="text-xs text-fg-faint mb-6">{t('forgot_password_sent_line2')}</p>
    <button
     type="button"
 ...
 if (needsSupport) {
  return cardShell(
   <section className="rounded border border-border-strong bg-surface p-8 shadow-md text-center">
    ...
    <h2 className="mb-2 text-base font-semibold text-fg">{t('forgot_password_no_email_title')}</h2>
    <p className="mb-6 text-sm text-fg-muted">{t('forgot_password_no_email_desc')}</p>
    <button
     type="button"
 ...
 return cardShell(
  <section className="rounded border border-border-strong bg-surface shadow-md">
   <div className="border-b border-border bg-surface-2 px-5 py-3">
    <h2 className="text-sm font-semibold text-fg-muted">{t('forgot_password_title')}</h2>
   </div>
   <div className="p-5">
    <p className="mb-4 text-sm text-fg-muted">{t('forgot_password_desc')}</p>
    <form className="space-y-4" onSubmit={(event) => void onSubmit(event)}>
     <div>
      ...
      {checkStatus === 'checking' ? (
       <p className="mt-1 text-xs text-fg-faint">{t('forgot_password_checking')}</p>
      ) : null}
      {checkStatus === 'found' ? (
       <p className="mt-1 text-xs text-good-text">{t('forgot_password_user_found')}</p>
      ) : null}
      {checkStatus === 'notfound' ? (
       <p className="mt-1 text-xs text-bad-text">{t('forgot_password_user_not_found')}</p>
      ) : null}
     </div>
