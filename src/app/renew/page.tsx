   ...
   <SiteLayout>
    <div className="flex flex-1 items-center justify-center p-4">
     <div className="w-full max-w-sm">
      <section className="rounded border border-border-strong bg-surface p-8 shadow-md text-center">
       <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-accent-weak text-4xl">⏳</div>
       <h2 className="mb-2 text-base font-semibold text-fg">{t('signup_pending_title')}</h2>
       <p className="mb-5 text-sm text-fg-muted">{t('signup_pending_body')}</p>
       <button
        type="button"
   ...
   <div className="flex flex-1 items-center justify-center p-4">
    <div className="w-full max-w-sm">
     <div className="mb-6 text-center">
      <p className="text-sm text-fg-faint">{t('renew_title')}</p>
     </div>
     ...
     <section className="rounded border border-border-strong bg-surface shadow-md">
      <div className="border-b border-border bg-surface-2 px-5 py-3">
       <h2 className="text-sm font-semibold text-fg-muted">{t('signup_payment_title')}</h2>
      </div>
      ...
      <form onSubmit={(e) => void onSubmit(e)} className="p-5 space-y-4">
       ...
       {paymentInfo?.configured ? (
        <>
         {paymentInfo.qrDataUrl ? (
          ...
          <img src={paymentInfo.qrDataUrl} alt="USDT wallet QR" className="mx-auto h-40 w-40 rounded border border-border bg-surface p-1" />
         ) : null}
         <div>
         ...
         <p className="rounded border border-amber-200 bg-warn-bg px-2 py-1.5 text-xs text-warn-text">
          {t('signup_payment_network_warning', { network: paymentInfo.network })}
         </p>
        </>
       ) : (
        <p className="rounded border border-amber-200 bg-warn-bg px-2 py-1.5 text-xs text-warn-text">{t('signup_payment_not_configured')}</p>
       )}
       ...
       {error && <p className="rounded border border-red-300 bg-bad-bg px-3 py-2 text-sm text-bad-text">{error}</p>}
       ...
       <div className="border-t border-border pt-4 text-center">
        <button type="button" onClick={() => router.push('/login')} className="text-sm text-accent transition hover:text-accent hover:underline">
         {t('login_title')}
        </button>
       </div>
      </form>
