...
export function Placeholder({ icon, bodyKey }: { icon: IconName; bodyKey: string }) {
 ...
 return (
  <div className="acp-card acp-card-pad" style={{ padding: '48px 24px', textAlign: 'center' }}>
   <div className="acp-kpi-ic" style={{ width: 46, height: 46, margin: '0 auto 14px' }}>
    <Icon name={icon} width={22} height={22} />
   </div>
   <div style={{ fontSize: 15, fontWeight: 650 }}>{t('admin_soon_title')}</div>
   <p className="acp-muted" style={{ fontSize: 13, marginTop: 8, maxWidth: 520, marginInline: 'auto' }}>
    {t(bodyKey)}
   </p>
  </div>
 );
