...
export function Avatar({ name, image, id, size = 34 }: { name: string; image?: string | null; id?: string; size?: number }) {
 if (image) {
  ...
  return <img src={image} alt={name} className="acp-avatar" style={{ width: size, height: size }} />;
 }
 return (
  <div className="acp-avatar" style={{ width: size, height: size, fontSize: Math.round(size * 0.38), background: avatarColor(id || name || '') }}>
   {getInitials(name)}
  </div>
 );
}
...
export function AuthBadge({ provider, t }: { provider: 'credentials' | 'oauth'; t: (k: string) => string }) {
 return provider === 'oauth' ? (
  <span className="acp-badge info">
   <span className="dot" />
   {t('admin_auth_google')}
  </span>
 ) : (
  <span className="acp-badge neutral">
   <span className="dot" />
...
export function RoleBadge({ role, t }: { role: string; t: (k: string) => string }) {
 ...
 return <span className={`acp-badge tiny ${owner ? 'warn' : 'neutral'}`}>{teamRoleLabel(role, t)}</span>;
}
...
export function RequestStatusBadge({ status, t }: { status: 'pending' | 'approved' | 'rejected'; t: (k: string) => string }) {
 ...
 return <span className={`acp-badge ${cls}`}>{label}</span>;
}
...
export function KpiCard({
 ...
 return (
  <div className="acp-kpi">
   <div className="acp-kpi-top">
    <div className={`acp-kpi-ic ${toneCls}`}>
     <Icon name={icon} />
    </div>
    <span className="acp-kpi-label">{label}</span>
   </div>
   <div className="acp-kpi-val acp-num">{value}</div>
   {foot && <div className="acp-kpi-foot">{foot}</div>}
  </div>
 );
}
...
export function StatTile({ label, value, sub }: { label: string; value: React.ReactNode; sub?: string }) {
 return (
  <div className="acp-card acp-card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
   <span className="acp-num" style={{ fontSize: 24, fontWeight: 700 }}>
    {value}
   </span>
   <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--ad-muted)' }}>{label}</span>
   {sub && <span className="acp-faint" style={{ fontSize: 12 }}>{sub}</span>}
  </div>
 );
}
...
export function StateBlock({ children }: { children: React.ReactNode }) {
 return <div className="acp-state">{children}</div>;
}

// ---------- Checkbox ----------
export function Check({ on, onClick }: { on: boolean; onClick: (e: React.MouseEvent) => void }) {
 return (
  <div className={`acp-chk ${on ? 'on' : ''}`} onClick={onClick} role="checkbox" aria-checked={on}>
...
export function RowMenu({ items, ariaLabel }: { items: RowMenuItem[]; ariaLabel?: string }) {
 ...
 return (
  <div ref={ref} style={{ position: 'relative', display: 'inline-block' }}>
   ...
   {open && (
    <div className="acp-menu" role="menu" onClick={(e) => e.stopPropagation()}>
     {items.map((item, i) => (
      <button
       key={i}
       type="button"
...
export function Modal({ onClose, children, maxWidth }: { onClose: () => void; children: React.ReactNode; maxWidth?: number }) {
 ...
 return (
  <div className="acp-modal-overlay" onClick={onClose}>
   <div className="acp-modal" style={maxWidth ? { maxWidth } : undefined} onClick={(e) => e.stopPropagation()}>
    {children}
   </div>
  </div>
 );
...
export function SectionHeader({ title, eyebrow, action }: { title: string; eyebrow?: boolean; action?: React.ReactNode }) {
 return (
  <div className="acp-section-head">
   <span className={eyebrow ? 'acp-eyebrow' : 'acp-section-title'}>{title}</span>
   {action}
  </div>
 );
}
