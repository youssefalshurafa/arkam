...
export function useReconciliationLocks({ reconciliations, clientAccountMap, lockPastEditsEnabled }: UseReconciliationLocksParams) {
 ...
 // Newest reconciliation per client account = the lock line used by the guards below. A
 // frozen historical checkpoint — depends only on `reconciliations`, never on the current
 // transaction list (see buildLockBoundaries).
